	...
	return this.each(function(){
		...
		var methods = {
			...
			go_to: function(index, easing, webkit){
				...
				if(settings.direction == 'vertical'){
					...
					anim_css = {'-moz-transform': 'translate3d(0px, ' + ((index - 1) * properties.height * (-1)) + 'px, 0px)'};
					anim_css = {'transform': 'translate3d(0px, ' + ((index - 1) * properties.height * (-1)) + 'px, 0px)'};
					anim_css = {'-ms-transform': 'translate3d(0px, ' + ((index - 1) * properties.height * (-1)) + 'px, 0px)'};
					anim_css = {'-o-transform': 'translate3d(0px, ' + ((index - 1) * properties.height * (-1)) + 'px, 0px)'};
					
				} else {
					
					anim = {'left': (index - 1) * properties.width * (-1)};
					anim_css = {'-webkit-transform': 'translate3d(' + ((index - 1) * properties.width * (-1)) + 'px, 0px, 0px)'};
					anim_css = {'-moz-transform': 'translate3d(' + ((index - 1) * properties.width * (-1)) + 'px, 0px, 0px)'};
					anim_css = {'transform': 'translate3d(' + ((index - 1) * properties.width * (-1)) + 'px, 0px, 0px)'};
					anim_css = {'-ms-transform': 'translate3d(' + ((index - 1) * properties.width * (-1)) + 'px, 0px, 0px)'};
					anim_css = {'-o-transform': 'translate3d(' + ((index - 1) * properties.width * (-1)) + 'px, 0px, 0px)'};
		...
		if($this.data('promptumenu')){
			...
			if(navigator.userAgent.toLowerCase().indexOf("mobile")>0){
				...
				$this.css('-webkit-transform', 'translate3d(' + settings.left + 'px, 0px, 0px)');
				$this.css('-moz-transform', 'translate3d(' + settings.left + 'px, 0px, 0px)');
				$this.css('transform', 'translate3d(' + settings.left + 'px, 0px, 0px)');
				$this.css('-ms-transform', 'translate3d(' + settings.left + 'px, 0px, 0px)');
				$this.css('-o-transform', 'translate3d(' + settings.left + 'px, 0px, 0px)');
				$this.data('ppos', settings.left);
			}else{
				$this.css({
					'left': settings.left
				});
			...
			$this.bind('mousedown.promptumenu', function(mdown){
				...
				$(document).bind('mouseup.promptumenu', function(mup){
					...
					if(settings.direction == 'vertical'){
						...
						//check if the user hasn't dragged over the end..
						if($this.children('li').width()-properties.width<=0){
							pos = 0;
						}else if(pos < ((-1) * ($this.children('li').width()-properties.width))){
							pos = (-1) * ($this.children('li').width()-properties.width);
						} else if(pos > 0){
							pos = 0;
						}
			...
			try {
				...
				var touchmove = function(tmove){
					...
					if(settings.direction == 'vertical'){
						...
						$this.css('-moz-transform', 'translate3d(0px, ' + (tinit_pos + tdelta.y) + 'px, 0px)');
						$this.css('transform', 'translate3d(0px, ' + (tinit_pos + tdelta.y) + 'px, 0px)');
						$this.css('-ms-transform', 'translate3d(0px, ' + (tinit_pos + tdelta.y) + 'px, 0px)');
						$this.css('-o-transform', 'translate3d(0px, ' + (tinit_pos + tdelta.y) + 'px, 0px)');
					} else {
						...
						$this.css('-moz-transform', 'translate3d(' + (tinit_pos + tdelta.x) + 'px, 0px, 0px)');
						$this.css('transform', 'translate3d(' + (tinit_pos + tdelta.x) + 'px, 0px, 0px)');
						$this.css('-ms-transform', 'translate3d(' + (tinit_pos + tdelta.x) + 'px, 0px, 0px)');
						$this.css('-o-transform', 'translate3d(' + (tinit_pos + tdelta.x) + 'px, 0px, 0px)');
					}
				...
				var touchend = function(tend){
					...
					if(delta_start==undefined){
						var med=tend.target.parentElement;
						if(med==null||med==undefined){
							return;
						}else{
							med=med.toString();
							var meo=med.split(":")[1];
							if(meo!=undefined&&meo!=null){
								setTimeout(meo,20);
					...
					if(settings.direction == 'vertical'){
						...
						if(settings.pages){
							...
							$this.css('-moz-transform', 'translate3d(0px, ' + pos + 'px, 0px)');
							$this.css('transform', 'translate3d(0px, ' + pos + 'px, 0px)');
							$this.css('-ms-transform', 'translate3d(0px, ' + pos + 'px, 0px)');
							$this.css('-o-transform', 'translate3d(0px, ' + pos + 'px, 0px)');
							$this.data('ppos', pos);
						}
					} else {
						...
						if(settings.pages){
							...
							$this.css('-moz-transform', 'translate3d(' + pos + 'px, 0px, 0px)');
							$this.css('transform', 'translate3d(' + pos + 'px, 0px, 0px)');
							$this.css('-ms-transform', 'translate3d(' + pos + 'px, 0px, 0px)');
							$this.css('-o-transform', 'translate3d(' + pos + 'px, 0px, 0px)');
							$this.data('ppos', pos);
							//alert("pos: "+pos);
						}
